...
function extractUserRoles(user) {
    ...
    if (user.role) {
        ...
        roles = roles.concat(Array.isArray(user.role) ? user.role : [user.role]);
    }
    // 2. Dari user.roles
    ...
    // 3. Dari resource_access (Keycloak client roles)
    ...
    // Hilangkan duplikat (gabungan realm + client roles)
    roles = [...new Set(roles)];
