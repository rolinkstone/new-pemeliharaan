...
/**
 * Ubah nama role teknis menjadi label yang enak dibaca.
 * contoh: admin_pemeliharaan -> Admin Pemeliharaan
 */
...
export default function DashboardLayout({ children }) {
  ...
  const getUserInfo = () => {
    ...
    // Tampilkan SEMUA role yang dimiliki user (bisa multi-role)
    const roles = (Array.isArray(session?.user?.roles) && session.user.roles.length > 0)
      ? session.user.roles
      : (session?.user?.role ? [session.user.role] : ['User']);

    return { name, email, initials, roles, roleLabel: roles.map(formatRoleLabel).join(' · ') };
  };

  const { name, email, initials, roles, roleLabel } = getUserInfo();
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-colors cursor-pointer group" onClick={() => setShowUserMenu(!showUserMenu)}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/40 truncate" title={roles.join(', ')}>{roleLabel}</p>
                </div>
              </div>
