...
import ImportRuanganModal from './modals/ImportRuanganModal';
...
const RuanganContainer = () => {
  ...
  // ========== INITIAL LOAD ==========
  // fetchDataRuangan ikut jadi dependency supaya perubahan filter/pencarian/pagination memicu muat ulang data
  useEffect(() => {
    if (session) {
      fetchDataRuangan();
    } else {
  ...
  const handleImportOpen = () => {
    if (isReadOnly()) {
      showSnackbar('Akses ditolak. Hanya admin_pemeliharaan dan admin yang dapat mengimport data ruangan.', 'error');
      return;
    }
    setImportModalOpen(true);
  };

  const handleImportSuccess = (message, data) => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const blob = await ruanganApi.exportRuanganXLSX(session);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `data-ruangan-${new Date().toISOString().slice(0, 10)}.xlsx`;
    ...
    <PolishedPageShell
      ...
      actions={
        <>
          ...
          <Tooltip title={isReadOnly() ? 'Hanya admin_pemeliharaan dan admin yang dapat mengimport data' : 'Import data ruangan dari file Excel'}>
            <span>
              <Button
                variant="outlined"
