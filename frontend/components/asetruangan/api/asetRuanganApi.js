...
/**
 * URL download template import posisi aset (endpoint publik, bisa dibuka via browser)
 */
export const downloadImportTemplateUrl = `${IMPORT_BASE_URL}/asetRuangan/import/template`;

/**
 * POST /api/asetRuangan/import
 * Import posisi aset ke ruangan dari file XLSX (dikirim sebagai base64)
 */
export const importAsetRuanganXLSX = async (session, file) => {
    const reader = new FileReader();
    return new Promise((resolve, reject) => {
        reader.onload = async () => {
            try {
                const base64 = reader.result.split(',')[1];
                const response = await fetch(`${IMPORT_BASE_URL}/asetRuangan/import`, {
                    method: 'POST',
                    headers: {
...
/**
 * GET /api/asetRuangan/export/xlsx
 * Export semua posisi aset sebagai file XLSX (Blob)
 */
export const exportAsetRuanganXLSX = async (session) => {
    const response = await fetch(`${IMPORT_BASE_URL}/asetRuangan/export/xlsx`, {
        method: 'GET',
...
export default {
    ...
    importAsetRuanganXLSX,
    exportAsetRuanganXLSX,
