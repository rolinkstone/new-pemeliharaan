...
import * as api from '../api/asetRuanganApi';

const ImportAsetRuanganModal = ({ open, onClose, session, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const res = await api.importAsetRuanganXLSX(session, file);
      setResult({
        ok: res?.success,
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 600 }}>
        Import Posisi Aset (XLSX)
        <Typography variant="caption" display="block" color="text.secondary" sx={{ mt: 0.5 }}>
          Tempatkan aset ke ruangan secara massal dari file Excel. Gunakan template untuk format yang benar.
        </Typography>
      </DialogTitle>
      ...
      <DialogContent>
        ...
        >
          <Box>
            <Typography variant="body2" fontWeight={600}>Template Import Posisi Aset</Typography>
            <Typography variant="caption" color="text.secondary">
              Kode Barang · NUP · Kode Ruangan · Tanggal Masuk
            </Typography>
          </Box>
          <Button
            size="small"
        ...
        <Alert severity="info" sx={{ mb: 2 }}>
          Baris aset yang sudah memiliki posisi <b>aktif</b> atau duplikat di dalam file akan
...
export default ImportAsetRuanganModal;
