...
/**
 * URL download template import Barang BMN (endpoint publik, bisa dibuka via browser)
 */
export const downloadImportTemplateUrl = `${IMPORT_BASE_URL}/aset/import/template`;

/**
 * POST /api/aset/import
 * Import Barang BMN dari file XLSX (dikirim sebagai base64)
 */
export const importAsetXLSX = async (session, file) => {
    const reader = new FileReader();
    return new Promise((resolve, reject) => {
        reader.onload = async () => {
            try {
                const base64 = reader.result.split(',')[1];
                const response = await fetch(`${IMPORT_BASE_URL}/aset/import`, {
                    method: 'POST',
                    headers: {
...
/**
 * GET /api/aset/export/xlsx
 * Export semua Barang BMN sebagai file XLSX (Blob)
 */
export const exportAsetXLSX = async (session) => {
    const response = await fetch(`${IMPORT_BASE_URL}/aset/export/xlsx`, {
        method: 'GET',
