...
import * as api from '../api/asetApi';

const ImportAsetModal = ({ open, onClose, session, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const res = await api.importAsetXLSX(session, file);
      setResult({
        ok: res?.success,
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 600 }}>
        Import Barang BMN (XLSX)
        <Typography variant="caption" display="block" color="text.secondary" sx={{ mt: 0.5 }}>
          Tambah data Barang Milik Negara secara massal dari file Excel. Gunakan template untuk format yang benar.
        </Typography>
      </DialogTitle>
      ...
      <DialogContent>
        ...
        >
          <Box>
            <Typography variant="body2" fontWeight={600}>Template Import Barang BMN</Typography>
            <Typography variant="caption" color="text.secondary">
              Jenis BMN · Kode Barang · NUP · Nama Barang · Merk · Kondisi · dll.
            </Typography>
          </Box>
          <Button
            size="small"
        ...
        <Alert severity="info" sx={{ mb: 2 }}>
          Kombinasi <b>kode_barang + nup</b> yang sudah ada atau duplikat di dalam file akan
...
export default ImportAsetModal;
