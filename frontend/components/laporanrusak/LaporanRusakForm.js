              ...
              {readOnly ? (
                ...
                <Autocomplete
                  ...
                  getOptionLabel={(opt) => {
                    if (!opt) return '';
                    const detail = [];
                    if (opt.nup !== undefined && opt.nup !== null && String(opt.nup).trim() !== '') {
                      detail.push(`NUP: ${opt.nup}`);
                    }
                    if (opt.merk) detail.push(`Merk: ${opt.merk}`);
                    return `${opt.kode_barang || ''} - ${opt.nama_barang || ''}${detail.length ? ` (${detail.join(' \u2022 ')})` : ''}`;
                  }}
                  value={selectedAset}
                    ...
                    <li {...props} key={opt.id}>
                      <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', py: 0.25 }}>
                        <Typography variant="body2" fontWeight={600}>
                          {opt.kode_barang} - {opt.nama_barang}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          NUP: {(opt.nup !== undefined && opt.nup !== null && String(opt.nup).trim() !== '') ? opt.nup : '-'}
                          {' \u2022 '}
                          Merk: {opt.merk || '-'}
                        </Typography>
                      </Box>
